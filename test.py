...
def batchProcess(inputFolder, outputFolder, readxml=False):
    ...
    for imagef in imageFiles:
        ...
        # imgname = "0"
        img = cv2.imread(inputFolder + "/" + imagef)

        # imageName = imagef.split('.')[0]
        ...
        if(readxml == True):
            print("calculate IOU using xml file")
            # BEGIN : SHOW XML
            ...
            # END : SHOW XML
        cv2.imwrite(outputFolder+"/"+imagef, img)
...
if __name__ == "__main__":
    batchProcess("data/private","data/output",False)
